...
def registration(request):
    ...
    if(password!=repeatedPassword):
        ...
        User.objects.create(name=name,surname=surname,username=username,email=email,typeOfUser=typeOfUser,phone=phone,password=make_password(password))
...
def login(request):
    ...
    if(User.objects.filter(email=email).exists())==False:
        return JsonResponse({'status':False,'message':'Korisnik sa unesenim emailom nije registrovan'},status=404)
    elif check_password(password,User.objects.get(email=email).password)==False:
        return JsonResponse({'status':False,'message':'Pogrešni kredencijali'},status=400)
    else:
        user=list(User.objects.filter(email=email).values('name','surname','email','phone','username','typeOfUser'))    
